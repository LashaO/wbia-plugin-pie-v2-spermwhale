...
def _read_and_validate_dataset_config(config_fpath):
    with open(config_fpath, "r") as stream:
        try:
            ...
            print(exc)
    assert (
        "data" in conf and "coco_dir" in conf["data"]
    ), "Error: ConfigDataset needs data.coco_dir to be defined in config file."
    return conf
...
class SpermWhaleMax10Test(COCODataset):
    def __init__(self, **kwargs):
        super(SpermWhaleMax10Test, self).__init__(
            name="spermwhale",
            dataset_dir="spermwhale",
            dataset_url="https://cthulhu.dyn.wildme.io/public/datasets/coco.sperm_whale.tar.gz",
            split="train2021",
