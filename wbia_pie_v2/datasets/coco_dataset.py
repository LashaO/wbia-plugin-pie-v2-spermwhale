...
class COCODataset(ImageDataset):
        ...
        for i, db_rec in enumerate(db_coco):
            ...
            if self.resize:
                ...
                if image.shape[0] <= image.shape[1]:
                    ...
                    out_size = (
                        int(image.shape[0] * min_size / image.shape[1]),
                        min_size,
                    )
    ...
    def _filter_min_samples(self, data, name_idx, min_samples, max_samples=None):
        ...
        print("filtering", min_samples, max_samples)
        ...
        if max_samples:
            filter_names = unames[(counts >= min_samples) & (counts <= max_samples)]
        else:
            filter_names = unames[counts >= min_samples]

        if max_samples:
            print(
                "Selecting names with more than {}, less than {} examples".format(
